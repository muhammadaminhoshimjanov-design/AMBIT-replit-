...
export function SummaryScreen() {
  ...
  async function handleEnter() {
    ...
    if (user) {
      const { error: err } = await supabase
        ...
        .upsert({
          ...
          bio: data.shortIntro,
          avatar_style: data.avatarStyle ?? "A",
          focus_topics: data.focusTopics,
          student_identity: data.studentIdentity,
          circle_preference: data.circlePreference,
    ...
    setSaving(false);
    // refreshProfile triggers auth state → onboarding_completed = true → MainAppScreen
  }
  ...
  return (
    <View style={styles.screen}>
      ...
      <Animated.View
        style={[styles.celebGlow, { opacity: heroOpacity, transform: [{ scale: ringScale }] }]}
      >
        <LinearGradient
      ...
      <View style={[styles.content, { paddingTop: topPad + 12, paddingBottom: bottomPad + 20 }]}>
        <ScrollView showsVerticalScrollIndicator={false}>
          <Animated.View
            style={[styles.heroSection, { opacity: heroOpacity, transform: [{ scale: heroScale }] }]}
          >
            <Animated.View
          ...
          <Animated.View
            style={[styles.summaryCard, { opacity: cardFade, transform: [{ translateY: cardSlide }] }]}
          >
            <LinearGradient
            ...
            >
              ...
              {data.circles.length > 0 && (
                <View style={styles.tagsBlock}>
                  <Text style={styles.infoKey}>Circles joined</Text>
                  <View style={styles.tagRow}>
                    {data.circles.map((c) => (
...
const styles = StyleSheet.create({
  ...
  celebGlow: {
    ...
    height: 400, borderRadius: 999, overflow: "hidden",
  },
  content: { flex: 1, paddingHorizontal: 24 },
  heroSection: {
    alignItems: "center", marginBottom: 28, paddingTop: 20, position: "relative",
  },
  ringOuter: {
  ...
  infoBadge: {
    backgroundColor: "rgba(99,102,241,0.14)", borderRadius: 10,
  ...
  editBtn: {
    ...
    gap: 7, paddingVertical: 14, marginBottom: 32,
  },
